...
def markdown_to_html_node(markdown_string):
    ...
    return MockHTMLNode(
        f"<div>{processed_html}</div>"
    )  # Ensure it returns a common structure
...
def generate_page(from_path, template_path, dest_path):
    ...
    try:
        ...
        print(
            f"Warning: Could not extract title from {from_path}: {e}. Using default or empty title."
        )
...
class TestGeneratePage(unittest.TestCase):
    ...
    def test_basic_page_generation(self):
        ...
        expected_html_content = markdown_to_html_node(
            "# My Awesome Page\n\nThis is **bold** and _italic_ content."
        ).to_html()
    ...
    def test_page_generation_with_subdir_creation(self):
        ...
        dest_file_path = os.path.join(
            self.dest_public_dir, "subdir", "nested_page.html"
        )
        generate_page(self.sample_md_path, self.sample_template_path, dest_file_path)
