...
def split_nodes_delimiter(old_nodes, delimiter, text_type):
    ...
    for old_node in old_nodes:
        ...
        if len(sections) % 2 == 0:
            raise ValueError(
                f"Invalid Markdown syntax: unterminated delimiter '{delimiter}'"
            )
        ...
        for i, section_text in enumerate(sections):
            ...
            if i % 2 == 0:
                split_nodes_for_current_node.append(
                    TextNode(section_text, TextType.TEXT)
                )
    ...
    match block_type:
        ...
        case "code":
            ...
            if content.startswith("```\n"):  # Handles ``` followed by newline
                ...
                "```"
            ):  # Handles ``` immediately followed by content
                content = content[3:]

            if content.endswith("```"):  # Handles content immediately followed by ```
                content = content[:-3]

            block_element = ParentNode(
                "pre", [text_node_to_html_node(TextNode(content, TextType.CODE))]
            )
        case "quote":
            ...
            for line in lines_in_block:
                if line.startswith("> "):
                    ...
                    ">"
                ):  # Handle cases like just ">" on an empty line if needed
        ...
        case "unordered_list":
            ...
            for line in lines:
                ...
                if (
                    content or line == "-"
                ):  # Handle cases like just "-" if that's valid.
        ...
        case "ordered_list":
            ...
            for i, line in enumerate(ordered_list):
                if line.startswith(f"{i + 1}. "):
                    ...
                    child_blocks.append(
                        ParentNode("li", text_to_children(cleaned_line))
                    )
